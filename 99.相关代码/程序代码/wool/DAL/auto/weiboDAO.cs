...
using wool.Utility;
namespace wool.DAO{
public partial class weiboDAO{
#region 向数据库中添加一条记录 +bool Insert(weibo model)
///<summary>
///向数据库中添加一条记录
///</summary>
...
{
const string sql = "DELETE FROM [dbo].[weibo] WHERE [objectId] = @objectId";
return SqlHelper.ExecuteNonQuery(sql, new SqlParameter("@objectId", objectId))>0;
}
#endregion
#region 根据主键ID更新一条记录 +bool Update(weibo model)
/// <summary>
/// 根据主键更新一条记录
/// </summary>
...
string sql = "SELECT COUNT(1) from weibo "+str;var res = SqlHelper.ExecuteScalar(sql, list.ToArray());
return res == null ? 0 : Convert.ToInt32(res);
}
#endregion
#region 查询单个模型实体 +weibo QuerySingleById(string objectId)
/// <summary>
/// 查询单个模型实体
/// </summary>
...
string[] clumns = new String[] { "objectId","uid","access_token","expires_in" };
string[] cls = columns.parseColumnsX(clumns,"weibo", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["weibo"])+" from weibo WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows)
{
...
///<returns>实体</returns>
public weibo QuerySingleByWheres(object wheres)
{
...
string[] clumns = new String[] { "objectId","uid","access_token","expires_in" };
string[] cls = columns.parseColumnsX(clumns,"weibo", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["weibo"])+" from weibo"+where;
...
string[] clumns = new String[] { "objectId","uid","access_token","expires_in" };
string[] cls = columns.parseColumnsX(clumns,"weibo", out li);
var sql = SqlHelper.GenerateQuerySql("weibo", li["weibo"], index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string sql = string.Format(@"UPDATE [dbo].[weibo] SET  {0}  WHERE [{1}] = @{1}", string.Join(",", column), "objectId");
...
list.AddRange(list1);
string sql = string.Format(@"UPDATE [dbo].[weibo] SET  {0} {1}", string.Join(",", column), where);
return SqlHelper.ExecuteNonQuery(sql, list.ToArray()) > 0;
}
 #endregion
}
