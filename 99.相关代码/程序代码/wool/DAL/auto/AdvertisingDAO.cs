...
using wool.Utility;
namespace wool.DAO{
public partial class AdvertisingDAO{
...
{
const string sql = "DELETE FROM [dbo].[Advertising] WHERE [objectId] = @objectId";
...
{
const string sql = @"UPDATE [dbo].[Advertising] SET  updatedAt=@updatedAt,createdAt=@createdAt,title=@title,url=@url,pic=@pic,state=@state  WHERE [objectId] = @objectId";
...
string sql = "SELECT COUNT(1) from Advertising "+str;var res = SqlHelper.ExecuteScalar(sql, list.ToArray());
...
{
const string sql = "SELECT TOP 1 objectId,updatedAt,createdAt,title,url,pic,state from Advertising WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows){
reader.Read();
Advertising model = SqlHelper.MapEntity<Advertising>(reader);
return model;
}
else
{
...
string[] clumns = new String[] { "objectId","updatedAt","createdAt","title","url","pic","state" };
string[] cls = columns.parseColumnsX(clumns,"Advertising", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["Advertising"])+" from Advertising WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows)
{
...
string[] clumns = new String[] { "objectId","updatedAt","createdAt","title","url","pic","state" };
string[] cls = columns.parseColumnsX(clumns,"Advertising", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["Advertising"])+" from Advertising"+where;
...
public IEnumerable<Advertising> QueryList(int index, int size, object wheres=null, string orderField="objectId", bool isDesc = true)
{
List<SqlParameter> list = null;
string where = wheres.parseWheres(out list);
orderField=string.IsNullOrEmpty(orderField) ? "objectId" : orderField;
var sql = SqlHelper.GenerateQuerySql("Advertising",new string[]{"objectId","updatedAt","createdAt","title","url","pic","state"}, index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string[] clumns = new String[] { "objectId","updatedAt","createdAt","title","url","pic","state" };
string[] cls = columns.parseColumnsX(clumns,"Advertising", out li);
var sql = SqlHelper.GenerateQuerySql("Advertising", li["Advertising"], index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string sql = string.Format(@"UPDATE [dbo].[Advertising] SET  {0}  WHERE [{1}] = @{1}", string.Join(",", column), "objectId");
...
list.AddRange(list1);
string sql = string.Format(@"UPDATE [dbo].[Advertising] SET  {0} {1}", string.Join(",", column), where);
return SqlHelper.ExecuteNonQuery(sql, list.ToArray()) > 0;
}
 #endregion
}
