...
using wool.Utility;
namespace wool.DAO{
public partial class OverageHistoryDAO{
#region 向数据库中添加一条记录 +bool Insert(OverageHistory model)
///<summary>
///向数据库中添加一条记录
///</summary>
///<param name="model">要添加的实体</param>
public bool Insert(OverageHistory model){
const string sql = @"INSERT INTO [dbo].[OverageHistory] (objectId,createdAt,updatedAt,userId,originUserId,type,before,after,change) VALUES (@objectId,@createdAt,@updatedAt,@userId,@originUserId,@type,@before,@after,@change)";
int res = SqlHelper.ExecuteNonQuery(sql,new SqlParameter("@objectId",model.objectId.ToDBValue()),new SqlParameter("@createdAt",model.createdAt.ToDBValue()),new SqlParameter("@updatedAt",model.updatedAt.ToDBValue()),new SqlParameter("@userId",model.userId.ToDBValue()),new SqlParameter("@originUserId",model.originUserId.ToDBValue()),new SqlParameter("@type",model.type.ToDBValue()),new SqlParameter("@before",model.before.ToDBValue()),new SqlParameter("@after",model.after.ToDBValue()),new SqlParameter("@change",model.change.ToDBValue()));
return res >0;
 }
#endregion
...
{
const string sql = "DELETE FROM [dbo].[OverageHistory] WHERE [objectId] = @objectId";
return SqlHelper.ExecuteNonQuery(sql, new SqlParameter("@objectId", objectId))>0;
}
#endregion
#region 根据主键ID更新一条记录 +bool Update(OverageHistory model)
/// <summary>
/// 根据主键更新一条记录
/// </summary>
/// <param name="model">更新后的实体</param>
/// <returns>是否成功</returns>
public bool Update(OverageHistory model)
{
const string sql = @"UPDATE [dbo].[OverageHistory] SET  createdAt=@createdAt,updatedAt=@updatedAt,userId=@userId,originUserId=@originUserId,type=@type,before=@before,after=@after,change=@change  WHERE [objectId] = @objectId";
...
string sql = "SELECT COUNT(1) from OverageHistory "+str;var res = SqlHelper.ExecuteScalar(sql, list.ToArray());
return res == null ? 0 : Convert.ToInt32(res);
}
#endregion
#region 查询单个模型实体 +OverageHistory QuerySingleById(string objectId)
/// <summary>
/// 查询单个模型实体
/// </summary>
/// <param name="id">objectId</param>);
/// <returns>实体</returns>);
public OverageHistory QuerySingleById(string objectId)
{
const string sql = "SELECT TOP 1 objectId,createdAt,updatedAt,userId,originUserId,type,before,after,change from OverageHistory WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows){
reader.Read();
OverageHistory model = SqlHelper.MapEntity<OverageHistory>(reader);
return model;
}
else
{
...
string[] clumns = new String[] { "objectId","createdAt","updatedAt","userId","originUserId","type","before","after","change" };
string[] cls = columns.parseColumnsX(clumns,"OverageHistory", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["OverageHistory"])+" from OverageHistory WHERE [objectId] = @objectId";
using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@objectId", objectId)))
{
if (reader.HasRows)
{
...
///<returns>实体</returns>
public OverageHistory QuerySingleByWheres(object wheres)
{
...
string[] clumns = new String[] { "objectId","createdAt","updatedAt","userId","originUserId","type","before","after","change" };
string[] cls = columns.parseColumnsX(clumns,"OverageHistory", out li);
string sql = "SELECT TOP 1 "+string.Join(",", li["OverageHistory"])+" from OverageHistory"+where;
...
public IEnumerable<OverageHistory> QueryList(int index, int size, object wheres=null, string orderField="objectId", bool isDesc = true)
{
List<SqlParameter> list = null;
string where = wheres.parseWheres(out list);
orderField=string.IsNullOrEmpty(orderField) ? "objectId" : orderField;
var sql = SqlHelper.GenerateQuerySql("OverageHistory",new string[]{"objectId","createdAt","updatedAt","userId","originUserId","type","before","after","change"}, index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string[] clumns = new String[] { "objectId","createdAt","updatedAt","userId","originUserId","type","before","after","change" };
string[] cls = columns.parseColumnsX(clumns,"OverageHistory", out li);
var sql = SqlHelper.GenerateQuerySql("OverageHistory", li["OverageHistory"], index, size, where, orderField, isDesc);
using (var reader = SqlHelper.ExecuteReader(sql,list.ToArray()))
{
if (reader.HasRows)
{
...
string sql = string.Format(@"UPDATE [dbo].[OverageHistory] SET  {0}  WHERE [{1}] = @{1}", string.Join(",", column), "objectId");
...
list.AddRange(list1);
string sql = string.Format(@"UPDATE [dbo].[OverageHistory] SET  {0} {1}", string.Join(",", column), where);
return SqlHelper.ExecuteNonQuery(sql, list.ToArray()) > 0;
}
 #endregion
}
